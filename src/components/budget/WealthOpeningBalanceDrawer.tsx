...
export function WealthOpeningBalanceDrawer({
  open,
  onClose,
  accounts,
  currency,
  ...
  focusAccountId,
  onSubmit,
}: WealthOpeningBalanceDrawerProps) {
  const visibleAccounts = useMemo(
    () => (focusAccountId ? accounts.filter((a) => a.id === focusAccountId) : accounts),
  ...
  useEffect(() => {
    ...
    setRows(
      visibleAccounts.map((a) => ({
        ...
        marketValue:
          a.market_value != null && Number(a.market_value) > 0
            ? String(Number(a.market_value))
            : "",
      }))
    );
              ...
              onSubmit={(e) => {
                ...
                const inputs: OpeningBalanceInput[] = rows
                  .map((row) => ({
                    ...
                    marketValue:
                      row.marketValue.trim() === "" ? null : Number(row.marketValue) || 0,
                ...
                {isOnboarding && (
                  <div className="border-primary/20 bg-primary/5 flex gap-3 rounded-lg border p-3">
                    ...
                    <p className="text-muted-foreground text-xs leading-relaxed">
                      ...
                      account. Skip any you don&apos;t use — you can update later. This is separate
                      from logging salary or monthly spending.
                    </p>
                  </div>
                )}
