...
function resolveAction(status: PaymentStatus, isAdmin: boolean): RowAction | null {
  if (isAdmin) {
    if (status === "pending") return { label: "Review",    nextStatus: null,     variant: "primary", opensModal: "review" };
    ...
    if (status === "unpaid")  return { label: "Submit Payment", nextStatus: null, variant: "primary", opensModal: "submit" };
  }
  return null;
}
        ...
        {action ? (
          ...
          <span
            className={cn(
              ...
              installment.status === "paid"    ? "bg-emerald-400"
              : installment.status === "pending" ? "bg-primary"
              : "bg-zinc-500"
            )}
          />
        )}
